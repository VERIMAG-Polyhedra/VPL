...
module Build (Min : Min.Type) = struct
	...
	(* let run : Objective.pivotStrgyT -> PSplx.t -> mapVar_t -> int -> int * (Region.t * Cons.t) list option
		= fun st sx map max_id -> *)
	...
	let build_norm_from_point : Cs.Vec.t -> 'c1 Cons.t list -> 'c2 Cons.t list -> Tableau.Vector.t
		...
		List.map
			...
			p1
		@ [Scalar.Rat.negU] (* Coeff of lambda_p1*)
	...
	let build_paramCoeff : V.t list -> 'c1 Cons.t -> PLP.ParamCoeff.t
		...
		let vec = List.map 
			(fun param ->
					Cons.get_c cons |> Cs.get_v |> fun vec -> Cs.Vec.get vec param) 
			params
		in
		PLP.ParamCoeff.mk vec (Cons.get_c cons |> Cs.get_c |> Cs.Coeff.neg)
	...
	let build_obj : V.t list -> 'c1 Cons.t list -> 'c2 Cons.t list -> PLP.Objective.t
		...
		let coeffs = List.map (build_paramCoeff params) p1
			@ [PLP.ParamCoeff.mk (List.map (fun _ -> Scalar.Rat.z) params) Scalar.Rat.negU]
	...
	let get_init_point : V.t list -> 'c1 Cons.t list -> 'c2 Cons.t list -> Cs.Vec.t 
		...
		(*
		let x1 = 
			match Opt.getAsg horizon (List.map (fun cons -> Cons.get_id cons, Cons.get_c cons) p1) with
			| None -> failwith "Unexpected empty polyhedron"
			| Some point -> Rtree.map Cs.Vec.ofSymbolic point
		and x2 = 
			match Opt.getAsg horizon (List.map (fun cons -> Cons.get_id cons, Cons.get_c cons) p2) with
			| None -> failwith "Unexpected empty polyhedron"
			| Some point -> Rtree.map Cs.Vec.ofSymbolic point
		in
		Cs.Vec.divc (Cs.Vec.add x1 x2) (Scalar.Rat.of_float 2.)
		*)
		(*
		match Splx.getAsgOpt horizon 
			(List.map (fun cons -> Cons.get_id cons, {(Cons.get_c cons) with Cs.typ = Cstr.Lt}) p1) 
		with
		| None -> begin
			match Splx.getAsgOpt horizon 
				(List.map (fun cons -> Cons.get_id cons, {(Cons.get_c cons) with Cs.typ = Cstr.Lt}) p2) 
			with
			| None -> begin
				let x1 = 
					match Opt.getAsg horizon (List.map (fun cons -> Cons.get_id cons, Cons.get_c cons) p1) with
...
module New2 = struct
	module Glpk = struct
		...
		module Float = Build(Min.RatVec_Glpk2(Vector.Float.Positive))

		module Symbolic = Build(Min.RatVec_Glpk2(Vector.Symbolic.Positive))
	end
	module Splx = struct
		module Rat = Build(Min.RatVec_Splx2(Vector.Rat.Positive))

		module Float = Build(Min.RatVec_Splx2(Vector.Float.Positive))

		module Symbolic = Build(Min.RatVec_Splx2(Vector.Symbolic.Positive))
	end
end
