...
module Min2 (VecInput : Vector.Type)(Vec : Vector.Type)(CsInput : Cstr.Type)(LP : MinLP.Type)  = struct
...
module Rat_Glpk2 = RatVec_Glpk2 (Cstr.Rat.Positive.Vec)

module Rat_Splx2 = RatVec_Splx2 (Cstr.Rat.Positive.Vec)

module Heuristic(Vec : Vector.Type with module M = Cstr.Rat.Positive.Vec.M) = struct
	let name = "Heuristic"
	module Vec = Vec
	
	module Glpk = RatVec_Glpk2(Vec)
	module Splx = RatVec_Splx2(Vec)
