	...
	module Monomial_t = struct
        ...
        let canonTs : Test.t
            ...
            [
                ...
                "mono0", ([x,1], Coeff.u), Some ([x,1], Coeff.u);
                "mono1", ([y,1], Coeff.u), Some ([y,1], Coeff.u);
                "mono2", ([x,1], Coeff.of_int 2), Some ([x,1], Coeff.of_int 2);
                "multi0", ([x,1; y,1], Coeff.u), Some ([x,1; y,1], Coeff.u);
                "multi1", ([y,1; x,1], Coeff.u), Some ([x,1; y,1], Coeff.u);
                "zero0", ([], Coeff.z), None;
                "zero1", ([], Coeff.z), None;
                "zero2", ([x,1], Coeff.z), None;(*
        ...
        let compare_ts : Test.t
            ...
            let tcs : (string * Poly.Monomial.t * Poly.Monomial.t * int) list =
                Poly.Monomial.([
                "equal_syn", canon ([x,1; y,1], Coeff.mk ~-2 3), canon ([x,1; y,1], Coeff.mk ~-2 3), 0;
                "equal", canon ([x,1; y,1], Coeff.mk ~-2 3), canon ([x,1; y,1], Coeff.of_int 12), 0;
                "neq", canon ([x,1; y,1], Coeff.mk ~-2 3), canon ([y,1], Coeff.mk ~-2 3), -1;
                ])
            in
        ...
        let equal_ts : Test.t
            ...
            let tcs : (string * Poly.Monomial.t * Poly.Monomial.t * bool) list =
                Poly.Monomial.([
                "equal", canon ([x,1; y,1], Coeff.mk ~-2 3), canon ([x,1; y,1], Coeff.mk ~-2 3), true;
                "neq0", canon ([x,1; y,1], Coeff.mk ~-2 3), canon ([x,1; y,1], Coeff.of_int 12), false;
                "neq1", canon ([x,1; y,1], Coeff.mk ~-2 3), canon ([y,1], Coeff.mk ~-2 3), false;
                ])
            in
        ...
        let mul_ts : Test.t
            ...
            let tcs : (string * Poly.Monomial.t * Poly.Monomial.t * Poly.Monomial.t) list = [
                "same_varset",
                ([x,1 ; y,1], Coeff.mk ~-2 3),
                ([x,1 ; y,1], Coeff.mk ~-2 3),
                ([x,2 ; y,2], Coeff.mk 4 9)
            ;
                "different_varset",
                ([x,2 ; z,3], Coeff.mk ~-2 3),
                ([y,4], Coeff.mk 4 5),
                ([x,2 ; y,4 ; z, 3], Coeff.mk ~-8 15)
            ;
                "some_common_varset",
                ([x,2 ; y,1 ; z,3], Coeff.mk ~-2 3),
                ([y,2], Coeff.mk 4 5),
                ([x,2 ; y,3 ; z, 3], Coeff.mk ~-8 15)
            ] in
	...
	module Poly_t = struct
        ...
        let monomial_coefficient_ts : Test.t
            ...
            let tcs : (string * Poly.t * Poly.MonomialBasis.t * Coeff.t) list = [
                "missing", Poly.of_string "-2/3*x1*x2", [x,1], Coeff.z;
                "found", Poly.of_string "-2/3*x1*x2", [x,1;y,1], Coeff.of_string "-2/3"
            ] in
        ...
        let monomial_coefficient_other_ts : Test.t
            ...
            let tcs : (string * Poly.t * Poly.MonomialBasis.t * Poly.t) list = [
                "full", Poly.of_string "-2/3*x1*x2", [x,1], [([y,1],Coeff.of_string "-2/3")];
                "partial", Poly.of_string "-2/3*x1*x2", [x,1;y,1], [([],Coeff.of_string "-2/3")]
            ] in
        ...
        let is_affine_ts : Test.t
            ...
            in [
                ...
                "linear1", true, Poly.mk_cste [[x,1], Coeff.u] Coeff.z;
                "linear2", true, Poly.mk_cste [[x,1], Coeff.u; [y,1], Coeff.of_int 3] Coeff.z;
                "affine", true, Poly.mk_cste [[x,1], Coeff.negU] Coeff.u;
                "affine2", true, Poly.mk_cste [[x,1], Coeff.negU; [z,1],Coeff.u] Coeff.u;
                "multi", false, Poly.mk_cste [[x,1; y,1], Coeff.u] Coeff.z;
                "mixed", false, Poly.mk_cste [[y,1; z,1], Coeff.of_int 2; [x,1], Coeff.u] Coeff.z
            ]
            |> List.map chk
        ...
        let of_string_ts : Test.t
            ...
            let tcs : (string * string * Poly.t) list = Poly.([
                ...
                "linear", "x1", mk_cste [[x,1], Coeff.u] Coeff.z;
                "product", "x1*x2", mk_cste [[x,1; y,1], Coeff.u] Coeff.z;
                "affine", "-1 * x1 + 1", mk_cste [[x,1], Coeff.negU] Coeff.u;
                "simple", "-2/3*x2*x1", mk_cste [[x,1; y,1], Coeff.mk 3 ~-2] Coeff.z;
            ]) in
