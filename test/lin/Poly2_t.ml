module Test (Vec : Vector.Rat.Type) = struct
	...
	let mkMB = Poly.MonomialBasis.mk_list
	let mkM (x,y) = Poly.Monomial.mk_list x y
	let mkP = Poly.mk_list
	...
	module Invariant_t
	  ...
	  module Monomial_t
		...
		let tcs : (string * bool * Poly.Monomial.t) list
		  ...
		  "multi0", false, ([x,1; x,1], Coeff.u);
		  "multi1", false, ([x,1; y,1; y,1], Coeff.u);
	...
	module Poly_t
	  ...
	  let canonTs : T.testT
		...
		then T.succeed state
		   ...
		   in
		   [
		"empty0", mkP [], mkP [[], Coeff.z];
		"empty1", mkP [[x], Coeff.z], mkP [[], Coeff.z];
	  ...
	  let monomial_coefficient_ts : T.testT
		...
		= [
		"missing", Poly.of_string "-2/3*x1*x2", mkMB [x], Coeff.z;
		"found", Poly.of_string "-2/3*x1*x2", mkMB [x;y], Coeff.of_string "-2/3"
		   ] in
	...
	= Poly.
	  ([
	    ...
	    "simple", "-2/3*x2*x1", mk_cste (mkP [[x; y], Coeff.mk 3 ~-2]) Coeff.z;
	  ]) in
