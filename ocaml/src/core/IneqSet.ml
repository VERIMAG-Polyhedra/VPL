...
module RmRedAux = struct
    ...
    let glpk: 'c t -> Scalar.Symbolic.t Rtree.t -> 'c t
        = fun s point ->
        let point = Rtree.map Cstr.Rat.Vec.ofSymbolic point in
		let cstrs = List.map Cons.get_c s.ineqs in
		let l = Min.Rat_Glpk.minimize point cstrs in
		let s' = List.filter (fun (cstr,_) ->
            List.exists (fun (cstr',_) -> Cstr.Rat.equalSyn cstr cstr') l
        ) s.ineqs in {
            regions = None;
            ineqs = s';
        }
