...
let rec (default : t)
	...
	and (powerLTOne : t)
		...
		| PowerLTOne mon ->
			(try 
				...
				let (bI,bounds) = (match HLP.run pn.Pneuma.lp pn.Pneuma.ph pn.Pneuma.sx pn.Pneuma.vl mon with
					| (Some bI,bounds) -> HOtypes.Debug.exec Debug.(bI,bounds) 
		...
		| _ -> Pervasives.failwith "Heuristic.powerLTOne"
	...
	and (extractEvenPowers : t)
		...
		| _ -> Pervasives.failwith "Heuristic.extractEvenPowers"
	...
	and (alreadyIn : t)
		...
		| _ -> Pervasives.failwith "Heuristic.AlreadyIn"
