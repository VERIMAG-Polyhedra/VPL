...
module Handelman (Minimization : Min.Type) = struct
	...
	module PLP_MODIF = struct
		...
		let choose_init_point : 'c HPol.t -> config -> Vec.t * config
			...
			then
				...
				match Region.getPointInside NCone horizon (ph#get_ineqs()) with
		...
		let init_region : 'c HPol.t -> Region.t list
			= fun ph ->
			let horizon = ph#horizon() in
			List.mapi
				(fun id ineq -> 
					match Region.getPointInside NCone horizon [ineq], Region.getPointInside NCone horizon [Cs.compl ineq] with
		...
		let run_plp : 'c HPol.t -> Obj.pivotStrgyT -> PSplx.t -> (Region.t * 'c PLPCore.Cons.t) list option
			= fun ph st sx -> 
			let config' = {std_config with
				reg_t = NCone;
				stgy = st;
				regions = init_region ph;
				} in
	...
	let handelman : Obj.pivotStrgyT -> 'c HPol.t -> Poly.t list -> Poly.t 
		...
		(*
		let (normalization, _) = Norm.get ph objective in
		Debug.log DebugTypes.Normal
			 (lazy ("Normalization : " ^ (Poly.to_string normalization)));
		*)
		try 
		begin
			let sx = Build.from_poly lpvars simplex_inequalities simplex_equalities objective None in
...
(*			Debug.log DebugTypes.Detail (lazy("Basis : " ^ (Misc.list_to_string PLP.PSplx.to_string sx)));*)
			try PLP_MODIF.run_plp ph st sx 
			with Failure s -> 
...
(*
	...
	*)
	...
	(*
	let handelman : Obj.pivotStrgyT -> 'c HPol.t -> Hi.t list -> Poly.t list -> Poly.t 
		-> (int * PLP.t) option
		= fun st ph his his_p f -> 
		let variables = ph#get_vars in
		let nb_h = (List.length his) in 
		let n = Var.Positive.next (Misc.max Poly.V.cmp variables) in
		let lpvars = List.map
			(fun i -> 
			(Misc.range n (nb_h + n)) in 
		(* on ajoute la contrainte triviale aux his*)
		(*XXX: apparemment ça sert à rien *)
		(*let his_p = (Poly.cste Scalar.Rat.u) :: his_p
		in*)
		(* le ième Hi est associé à la variable numéro i*)
		let flin = 	(List.fold_right 
				(fun i p -> Poly.add 
				...
				f) 
		in
		let map = init_map nb_h in
		let max_id = nb_h in
		let simplex_equalities = get_non_linear_coeffs flin variables in
		let simplex_inequalities = [] in
		let objective = Poly.get_affine_part flin variables in 
		Debug.log(lazy("Objective = " ^ (Poly.to_string objective)));
		let (normalization, _) = Norm.get ph objective in
		Debug.log (lazy ("Normalization : " ^ (Poly.to_string normalization)));
		try 
		begin
			let sx = Build.from_poly lpvars simplex_inequalities simplex_equalities objective normalization in
			Debug.log(lazy("Simplex tableau : \n" ^ (PSplx.to_string sx)));
			try 
				Some (Debug.exec (P.run st sx map max_id)
					(lazy("PLP ended")))
			with 
				Failure s -> (Debug.log(lazy("Handelman Exception: " ^ s)); None)
		end
		with
			Invalid_argument s -> Debug.exec None (lazy("Handelman_dicho : invalid_argument -> " ^ s))
	*)
