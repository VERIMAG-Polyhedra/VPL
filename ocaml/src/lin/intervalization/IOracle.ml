...
let rec(factorize : IHeuristic.prophecy -> ASTerm.BasicZTerm.term)
	...
	then Term.zero
		...
		(l1 |> List.map (fun y -> try Term.get_affine_part y with 
			Not_found -> if ASTerm.BasicZTerm.isCte t then t else Pervasives.failwith (Printf.sprintf "factorisation : %s -> non-constant term with no affine part" (Term.to_string t)))
		|> List.fold_left
		ASTerm.BasicZTerm.smartAdd
		Term.zero)))
