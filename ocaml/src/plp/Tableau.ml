...
module Vector
  ...
  let size : t -> int
    ...
    if n > 0 then n
    else Pervasives.invalid_arg "Matrix.Vector.size: empty"
...
[Invalid_argument] is raised. *)
  let init : int -> (int -> Q.t) -> t
    = fun i f ->
    if i < 1 then Pervasives.invalid_arg "Matrix.Vector.init"
...
module Matrix
  ...
  let nRows : t -> int
    ...
    if n > 0 then n
    else Pervasives.invalid_arg "Matrix.Matrix.nRows: empty"
