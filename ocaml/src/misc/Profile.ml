			...
			(fun name e (max_name, max) ->
				...
				then match e.start, max.start with
					...
					| Some e_start, Some max_start -> 
						if Z.geq e_start max_start
						then (name, e)
						else (max_name, max)
				else (max_name, max))
			!map ("", {e0 with start = Some Z.zero})
		in
		max_name
		...
		then begin
			...
			try 
				...
				| Some start_time ->
					...
					map := MapPro.add name {start = None ; time = Z.add time' e.time} !map
...
module Report = struct
	...
	let sort_tree_list : tree list -> tree list
		= List.fast_sort
			(fun (Node((_,t1),_)) (Node((_,t2),_)) -> Z.compare t1 t2)
