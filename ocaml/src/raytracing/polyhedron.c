...
bool Polyhedron::GetExactSolution (const VectorZ& obj, int objdir) {
  ...
  bool glpkRes = glpkInter.Simplex(*this, obj, objdir) ;
  if (glpkRes == false) {
    std::cout << "LP has no optimal solution." << std::endl << std::endl ;
    return false ;
  }
  glpkInter.GetBasis() ;
  ...
  //print(newObj) ;
  // check result
  std::vector<int> nonbasic = glpkInter.GetNonbasicIdx() ; 
  RNumber zero ;
  zero.set_zero() ;
  int currIdx, state ;
